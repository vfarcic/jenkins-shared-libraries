def call(String project, String hubUser, String hostIp, String basePath) {
    script {
        try {
            ...
            sh "TAG=${env.BUILD_NUMBER} HOST_IP=${hostIp} SERVICE_PATH=${basePath}-${env.BRANCH_NAME}-${env.BUILD_NUMBER} docker-compose -p ${project}-${env.BRANCH_NAME}-${env.BUILD_NUMBER} run --rm production"
        } catch (e) {
